...
namespace MyFramework.Data
{
    ...
    public sealed class DataHelper
    {
        ...
        public string ParameterSeparateCode
        {
            get
            {
               return "?";
        ...
        public DataHelper()
        {
            //db = new MyFramework.Data.Database();
        }
        ...
        /// <param name="SQLString">SQL语句</param> 
        /// <returns>影响的记录数</returns> 
        public int ExecuteNonQuery(MySqlCommand dbCommand)
        {
            try
            {
        ...
        public object ExecuteScalar(DbCommand dbCommad)
        {
            try
            {
                dbCommad.Connection.Open();
                object obj = dbCommad.ExecuteScalar();
                if ((Object.Equals(obj, null)) || (Object.Equals(obj, System.DBNull.Value)))
                {
                    return null;
                }
        ...
        /// <returns></returns> 
        public DataSet ExecuteDataSet(string cmdText, int pageIdex, int pageSize, ref int recordCount)
        {
            int rows = 0;
            DataSet ds = new DataSet();
            MatchCollection matchs = Regex.Matches(cmdText, @"top\s+\d{1,}", RegexOptions.IgnoreCase);//含有top 
            if (matchs.Count > 0) //含有top的时候 
            {
                DataTable dtTemp = ExecuteDataSet(cmdText).Tables[0];
                rows = dtTemp.Rows.Count;
            }
            else //不含有top的时候 
            {
                //获取行数 
                object obj = ExecuteScalar(cmdText);
                if (obj != null)
                {
                    rows = Convert.ToInt32(obj);
                }
            }
            ds = ExecuteDataSet(cmdText, (pageIdex - 1) * pageSize, pageSize);
            recordCount = rows;
            return ds;
        }
        #endregion
        ...
        /// <param name="pageSize">每页记录数</param> 
        /// <returns></returns> 
        public DataSet getPager(out int recordCount, string selectList, string tableName, string whereStr, string orderExpression, int pageIdex, int pageSize)
        {
            int rows = 0;
            DataSet ds = new DataSet();
            ...
            ds = ExecuteDataSet(sqlStr, (pageIdex - 1) * pageSize, pageSize);
            recordCount = rows;
            return ds;
        }
        #endregion
        ...
        /// <returns></returns>
        public MySqlCommand GetSqlStringCommand(string SqlCmd) 
        {
            MySqlConnection connection = new MySqlConnection(connectionString);
            MySqlCommand myCommand = new MySqlCommand(SqlCmd,connection);
        ...
        /// <returns></returns>
        //public DbCommand GetStoredProcCommand(string storedProcedureName) { return db.GetStoredProcCommand(storedProcedureName); }
        /// <summary>
        /// 生成存储过程命令对象
        /// </summary>
        /// <param name="storedProcedureName">存储过程名称</param>
        /// <param name="parameterValues">存储过程参数集</param>
        /// <returns></returns>
        //public DbCommand GetStoredProcCommand(string storedProcedureName, params object[] parameterValues) { return db.GetStoredProcCommand(storedProcedureName, parameterValues); }
        #endregion
        ...
        public void AddInParameter(MySqlCommand command, string name, DbType dbType)
        {
        ...
        public void AddInParameter(MySqlCommand command, string name, DbType dbType, object value)
        {
        ...
        public void AddInParameter(MySqlCommand command, string name, DbType dbType, string sourceColumn, DataRowVersion sourceVersion)
        {
        ...
        public void AddOutParameter(MySqlCommand command, string name, DbType dbType, int size)
        {
        ...
        /// <param name="sourceVersion"></param>
        /// <param name="value"></param>
        public void AddParameter(MySqlCommand command, string name, DbType dbType, int size, ParameterDirection direction, bool nullable, byte precision, byte scale, string sourceColumn, DataRowVersion sourceVersion, object value)
        {
        ...
        /// <param name="sourceVersion"></param>
        /// <param name="value"></param>
        public void AddParameter(MySqlCommand command, string name, DbType dbType, ParameterDirection direction, string sourceColumn, DataRowVersion sourceVersion, object value)
        {
        ...
        /// <param name="value"></param>
        public void SetParameterValue(MySqlCommand command, string parameterName, object value)
        {
            if (command == null) throw new ArgumentNullException("command");
        ...
        /// <param name="name"></param>
        /// <returns></returns>
        public object GetParameterValue(MySqlCommand command, string name)
        {
            if (command == null) throw new ArgumentNullException("command");
        ...
        private DbParameter CreateParameter(string name)
        {
            DbParameter param = MySql.Data.MySqlClient.MySqlClientFactory.Instance.CreateParameter();
